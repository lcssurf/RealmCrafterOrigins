...
if exist "build\CMakeCache.txt" (
    ...
    if errorlevel 1 (
        echo [RCO] Existing CMake cache uses another generator. Recreating GUE build folder...
        rmdir /s /q build
    ) else (
        findstr /I /C:"CMAKE_GENERATOR_PLATFORM:INTERNAL=x64" "build\CMakeCache.txt" >nul 2>&1
        if errorlevel 1 (
            echo [RCO] Existing CMake cache uses another platform. Recreating GUE build folder...
            rmdir /s /q build
        )
    )
)
