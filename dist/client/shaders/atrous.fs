...
void main()
{
  ...
  vec4 pval = vec4(WorldPosFromDepth(texture(gDepth, vTexCoord).r, u_resolution, u_invViewProj), 1.0);
  ...
  for (int i = 0; i < 25; i++)
  {
    ...
    vec4 ptmp = vec4(WorldPosFromDepth(texture(gDepth, uv).r, u_resolution, u_invViewProj), 1.0);
    t = pval - ptmp;
    dist2 = dot(t, t);
